...
def combine_images(
    ...
    def remove_background(image):
        ...
        threshold = (
            0  # Adjust this value to control the sensitivity of background removal
        )
        ...
        for pixel in pixels:
            if (
                abs(pixel[0] - background_color[0]) <= threshold
                and abs(pixel[1] - background_color[1]) <= threshold
                and abs(pixel[2] - background_color[2]) <= threshold
            ):
    ...
    if mode == "horizontal":
        combined_width = sum(
            Image.open(image).size[0] for image in images
        ...
        combined_height = sum(
            Image.open(image).size[1] for image in images
        ) + spacing * (len(images) - 1)
    else:
    ...
    if spacing_color == "transparent":
        ...
        combined_image = Image.new(
            "RGB", (combined_width, combined_height), spacing_color
        )
